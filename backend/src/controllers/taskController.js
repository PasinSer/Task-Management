...
exports.getAllTasks = catchAsync(async (req, res, next) => {
  const สิ่งที่ต้องดำเนินการ = await Task.find({ user: req.user, status: "สิ่งที่ต้องดำเนินการ" }).sort({
    order: 1,
  });
  const รอดำเนินการ = await Task.find({
    user: req.user,
    status: "รอดำเนินการ",
  }).sort({ order: 1 });
  const เสร็จสิ้น = await Task.find({
    user: req.user,
    status: "เสร็จสิ้น",
  }).sort({ order: 1 });
  res.status(200).json({
    status: "success",
    user: req.user,
    tasks: { สิ่งที่ต้องดำเนินการ, รอดำเนินการ, เสร็จสิ้น },
  });
});
